...
longcorgi_url = "https://i.imgur.com/BYsRQYM.jpg"
...
theteam_url = "https://i.imgur.com/BYsRQYM.jpg"
...
md"""
# Cosas divertidas: jugar con transformaciones
"""
...
md"""
# Las sobras de la última clase
"""

# ╔═╡ 78d61e28-79f9-11eb-0605-e77d206cda84
md"""
## Pregunta interesante sobre las transformaciones lineales

Si una transformación toma líneas en líneas y conserva el origen, ¿tiene que ser lineal?

¡Respuesta = **no**!

El ejemplo de un **mapa de perspectiva** convierte todas las líneas en líneas, pero los paraleogramas generalmente no se convierten en paralelogramos. 
"""

# ╔═╡ aad4d6e4-79f9-11eb-0342-b900a41cfbaf
md"""
[Una buena demostración interactiva de mapas en perspectiva] (https://www.khanacademy.org/humanities/renaissance-reformation/early-renaissance1/beginners-renaissance-florence/a/linear-perspective-interactive) de la academia Khan.
"""
...
md"""
**Ejercicio de desafío**: ¡Reescribe esto usando Julia y Plutón!"""

# ╔═╡ a0afe3ae-76b9-11eb-2301-cde7260ddd7f
md"""
# Transformaciones lineales: una colección"""

# ╔═╡ fc2deb7c-7aa1-11eb-019f-d3e3c80b9ff1
md"""
Aquí hay algunas transformaciones lineales útiles:"""
...
md"""
La segunda versión utiliza la notación de multiplicación matricial del álgebra lineal, lo que significa exactamente lo mismo que la primera versión cuando
...
md"""
# Transformaciones no lineales: una colección"""

# ╔═╡ b4cdd412-7a02-11eb-149a-df1888a0f465
begin
  translate(α,β)  = ((x, y),) -> SA[x+α, y+β]   # Afín, pero no lineal
...
md"""
### Encuentra tus propios ejemplos

Tome algunas de las transformaciones lineales y no lineales (consulte la tabla de contenido) y encuentre algunos inversos colocándolos en la sección `T=` de "Las cosas divertidas" en la parte superior de este cuaderno."""

# ╔═╡ 89f0bc54-76bb-11eb-271b-3190b4d8cbc0
md"""
Las transformaciones lineales se pueden escribir en matemáticas usando la notación de multiplicación de matrices como 
...
md"""
Por el contrario, aquí hay algunas funciones divertidas que no se pueden escribir como matriz por vector. ¿Qué caracteriza a los de matriz de los que no son de matriz?
"""
...
md"""
# Transformaciones lineales: vea una matriz, piense más allá de las matrices numéricas"""

# ╔═╡ e051259a-7a74-11eb-12fc-99c5dc867fbd
md"""
Los estudiantes principiantes de álgebra lineal ven una matriz y ven una tabla con números. Queremos que veas una *transformación lineal*, eso es lo que ven los matemáticos profesionales.
"""

# ╔═╡ 1856ddae-7a78-11eb-3422-298e1103275b
md"""
¿Qué define una transformación lineal? Hay algunas formas equivalentes de dar una definición.
"""

# ╔═╡ 4b4fe818-7a78-11eb-2986-59e60063d346
md"""
**Definiciones de transformación lineal:**"""

# ╔═╡ 5d656494-7a78-11eb-12e8-d17856bd8c4d
md"""
- La definición intuitiva:

> Los rectángulos (líneas de cuadrícula) en la imagen transformada [arriba] (#e0b657ce-7a03-11eb-1f9d-f32168cb5394) siempre se convierten en una red de paralelogramos congruentes.

- La definición fácil de operar (pero desprovista de intuición):

> Una transformación es lineal si está definida por $v \mapsto A*v$ (matriz veces vector) para alguna matriz fija $A$.

- La definición de escalado y adición:

> 1. Si escalas y luego transformas o si transformas y luego escalas, el resultado es siempre el mismo:
> $T(cv)=c \, T(v)$
> ($v$ es cualquier vector y $c$ cualquier número.)

> 2. Si añades y luego transformas o viceversa, el resultado es el mismo:
> $T(v_1+v_2) = T(v_1) + T(v_2).$ ($v_1,v_2$ son cualquier vector.)

- La definición del matemático (consolidación de la definición anterior):
> ``T`` es lineal si 
> ``T(c_1 v_1 + c_2 v_2) = c_1 T(v_1) + c_2 T(v_2)``
> para todos los números $c_1,c_2$ y vectores $v_1,v_2$ (esto se puede extender a más de 2 términos).
...
md"""
La matriz de una transformación lineal $T$ es fácil de escribir: La primera columna es $T([1, 0])$ y la segunda es $T([0,1])$. ¡Eso es todo!
"""

# ╔═╡ c9f2b61e-7a84-11eb-3841-33739a226ff9
md"""
Una vez que los tenemos, la relación de linealidad

$$T([x,y]) = x \, T([1,0]) + y \, T([0,1]) = x \, \mathrm{(column\ 1)} + y \, \mathrm{(column\ 2)}$$

Es exactamente la definición del vector por matriz. Pruébalo.

"""

# ╔═╡ 23d8a45c-7a85-11eb-3a68-ef11e6f58cac
md"""
### Multiplicación matricial: sabes cómo hacerlo, pero ¿por qué?"""

# ╔═╡ 4a96d516-7a85-11eb-181c-63a6b461790b
md"""
¿Alguna vez te has preguntado por qué la multiplicación de la matriz tiene esa complejidad de multiplicar y sumar?"""
...
md"""
**Importante:** ¡La composición de la transformación lineal es la transformación lineal de las matrices multiplicadas! Solo hay una definición de matmul (matriz multiplicación) que se da cuenta de este hecho.

Para ver qué es exactamente, recuerda que la primera columna de `lin(A) ∘ lin(B)` debería ser el resultado de calcular dos matrices por vectores $y=B*[1,0]$ y luego $z=A*y$ y la segunda columna es la misma para $[0,1]$.

Vale la pena escribir esto si nunca lo has hecho.
"""

# ╔═╡ 17281256-7aa5-11eb-3144-b72777334326
md"""
Intentemos hacer eso con matrices aleatorias:"""
...
begin
	...
	lin(P*Q)((1, 0)), (lin(P)∘lin(Q))((1, 0)), (lin(Q)∘lin(P))((1, 0))
end
...
# ╔═╡ f98b874e-944a-4366-8ef1-0226ed65a9e5
md"""
`lin(Q)∘lin(P)`
"""
...
md"""
Si $f$ es una función de 2-vectores a 2-vectores (digamos), definimos el **inverso** de $f$, denotado $f^{-1}$, para tener la propiedad de que "*deshace*" el efecto de $f$, es decir,
...
Esta ecuación podría ser cierta para todos los $v$ o para algunos $v$ en una región.
"""

# ╔═╡ 7a4e785e-7a71-11eb-07fb-cfba453a117b
md"""
## Ejemplo: Escalar hacia arriba y hacia abajo"""
...
md"""
Observamos numéricamente que "escala(2)" y "escala(.5)" son transformaciones mutuamente inversas, es decir, cada una es la inversa de la otra.
"""

# ╔═╡ c7cc412c-7aa5-11eb-2df1-d3d788047238
md"""
## Inversos: Resolviendo ecuaciones"""

# ╔═╡ ce620b8e-7aa5-11eb-370b-11e34b07d54d
md"""
¿Qué hace realmente una inversa?

Pensemos en escalar de nuevo. Supongamos que escalamos un vector de entrada $\mathbf{x}$ por 2 para obtener un vector de salida $\mathbf{y}$:

$$\mathbf{y} = 2 \mathbf{x}$$

Ahora supongamos que quieres ir hacia atrás. Si se te da $\mathbf{y}$, ¿cómo encuentras $\mathbf{x}$? En este caso en particular, vemos que $\mathbf{x} =\frac{1}{2} \mathbf{y}$.

Si tenemos una transformación *lineal*, podemos escribir

$$\mathbf{y} = A \, \mathbf{x}$$

con una matriz $A$.

Si nos dan $\mathbf{y}$ y queremos retroceder para encontrar el $\mathbf{x}$ a partir de eso, necesitamos *resolver un sistema de ecuaciones lineales*.

*Por lo general*, pero *no siempre*, podemos resolver estas ecuaciones para encontrar una nueva matriz $B$ tal que

$$\mathbf{x} = B \, \mathbf{y},$$

Es decir, $B$ *deshacer* el efecto de  $A$. Entonces necesariamente $B * A$ es la matriz de identidad. Llamamos a $B$ la *matriz inversa* de $A$, y escribimos

$$B = A^{-1}.$$

Para $2 \times 2$ matrices podemos escribir una fórmula explícita para el inverso de la matriz, pero en general necesitaremos un ordenador para ejecutar un algoritmo para encontrar el inverso.
...
md"""
### Invirtiendo transformaciones no lineales"""

# ╔═╡ 0be9fb1e-7aa7-11eb-0116-c3e86ab82c77
md"""
¿Qué pasa si tenemos una transformación *no lineal* $T$ -- ¿podemos invertirla? En otras palabras, si $\mathbf{y} = T(\mathbf{x})$, ¿podemos resolver esto para encontrar $\mathbf{x}$ en términos de $\mathbf{y}$?

¡En general, esta es una pregunta difícil! A veces podemos hacerlo analíticamente, pero por lo general no podemos.

Sin embargo, hay métodos *numéricos* que a veces pueden resolver estas ecuaciones, por ejemplo, el [método Newton] (https://en.wikipedia.org/wiki/Newton%27s_method).

Hay varias implementaciones de tales métodos en Julia, por ejemplo, en el [paquete NonlinearSolve.jl] (https://github.com/JuliaComputing/NonlinearSolve.jl). Lo hemos utilizado para escribir una función "inversa" que intenta invertir las transformaciones no lineales de nuestras imágenes.
"""

# ╔═╡ 5f0568dc-7aad-11eb-162f-0d6e26f17d59
md"""
Tenga en cuenta que estamos definiendo el mapa con el inverso de T para que podamos ir píxel a píxel en el resultado.
"""
...
[Aplicación lineal Wikipedia](https://en.wikipedia.org/wiki/Linear_map)

[Matriz de transformación lineal Wikipedia](https://en.wikipedia.org/wiki/Transformation_matrix)
"""
...
# ╔═╡ 7c696b82-0369-4d80-8e25-c2a2ba84fbbb
begin
		[			    
			begin
			 x, y = transform_ij_to_xy(i,j, test_pixels)
